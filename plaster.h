...
//	~plaster(){control_output<<"plaster destructed"<<endl;};
	...
	int choose_typ( const vector <unsigned int> &exclude= vector<unsigned int>());
	...
	void	delete_site(unsigned int typ, unsigned long numer){
		...
		if( (st_size - PL_SITES_TYP[typ].size()) != 1){
			...
			control_output<<st_size<<" "<<PL_SITES_TYP[typ].size()<<" "<<(st_size - PL_SITES_TYP[typ].size())<<" "<<(st_size - PL_SITES_TYP[typ].size())<<typ<<endl;
			show();
			exit(1);		
		}
	...
	void	add_site(int typ, site* new_site){
		...
		if( ( PL_SITES_TYP[typ].size() - st_size ) != 1){
			...
			control_output<<st_size<<" "<<PL_SITES_TYP[typ].size()<<" "<<( PL_SITES_TYP[typ].size() - st_size )<<" "<<(PL_SITES_TYP[typ].size() - st_size)<<typ<<endl;
			show();
			exit(1);		
		}
