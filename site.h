...
std :: multimap<int, std::vector<site*> > clusters; 
 // site internal container to keeps/reuse calculated clusters [cluster_size][cluster_number_of_that_size][site_in_cluster]=site
...
void calc_clusters(int cs, std::vector <std::vector <<site*> > &clusters_of_size);
